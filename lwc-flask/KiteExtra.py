...
class KiteExtTicker(KiteTicker):
    ...
    def _subscribe(self, instrument_tokens):
        reactor.callFromThread(self.subscribe, instrument_tokens)

    def _unsubscribe(self, instrument_tokens):
        reactor.callFromThread(self.unsubscribe, instrument_tokens)

    def setMode(self, mode, instrument_tokens):
        reactor.callFromThread(self.set_mode, mode, instrument_tokens)

    def UnsubSubSetMode(self, prev_instrument_tokens, instrument_tokens, mode):
        self._unsubscribe(prev_instrument_tokens)
        self._subscribe(instrument_tokens)
...
class KiteExt(KiteConnect):
    def login_with_credentials(self, userid, password, pin):
        self.headers = {
            "x-kite-version": "3",
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.128 Safari/537.36",  # noqa: E501
        }
        self.user_id = userid
    ...
    def set_headers(self, enctoken, userid=None):
        ...
        if self.user_id is None:
            raise Exception(
                f"userid cannot be none, either login with credentials first or set userid here"  # noqa: E501
            )
        self.headers = {
            "x-kite-version": "3",
        ...
        if "json" in r.headers["content-type"]:
            try:
                ...
                raise ex.DataException(
                    "Could not parse the JSON response received from the server: {content}".format(  # noqa: E501
                        content=r.content
                    )
                )
            ...
            raise ex.DataException(
                "Unknown Content-Type ({content_type}) with response: ({content})".format(  # noqa: E501
